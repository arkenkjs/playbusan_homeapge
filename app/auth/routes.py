...
from app.models import User, AuthSMS, Position, positions_users
...
@bp.route('/register_test/<positions>/', methods=['GET', 'POST'])
def register_test(positions):
    form = RegistrationForm()
    form.positions.data = positions
    
    if positions == 'student':
        position = Position.query.filter(Position.positions == 'elementary').first()
    else:
        position = Position.query.filter(Position.positions == positions).first()    
    if form.validate_on_submit():
        user = User(username=form.username.data, email=form.email.data, birthday=form.birthday.data)
        user.set_password(form.password.data)
